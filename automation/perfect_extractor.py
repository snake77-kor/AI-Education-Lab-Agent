...
def exact_pipeline():
    ...
    with sync_playwright() as p:
        ...
        questions = [23, 24]
        for q in questions:
            print(f"[{q}번 문제] 1. 페이지 접속...")
            page.goto(notebook_url)
            time.sleep(10)
            
            print(f"[{q}번 문제] 2. 소스 단독 선택...")
            page.evaluate('''() => {
            ...
            print(f"[{q}번 문제] 5. 메모 이름 변경 및 소스로 전환...")
            try:
                page.evaluate('''() => {
            ...
            time.sleep(4)
            print(f"[{q}번 문제] 6. 슬라이드 생성 파이프라인 (소스 단독 선택)...")
            ...
            time.sleep(30)
            print(f"[{q}번 문제] 7. 슬라이드 이름 동기화 중...")
