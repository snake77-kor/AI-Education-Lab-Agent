...
def restore_and_extract():
    ...
    with sync_playwright() as p:
        ...
        questions = [23, 24]
        for q in questions:
            print(f"[{q}번 문제] 1. 페이지 로드 중...")
            page.goto(notebook_url)
            time.sleep(10)

            # Close studio or viewer if open
            page.evaluate('''() => {
                const closeBtn = Array.from(document.querySelectorAll('button')).find(b => 
                    b.getAttribute('aria-label') === '보기 닫기' || 
                    b.getAttribute('aria-label') === 'Close view' || 
                    b.getAttribute('aria-label') === 'Close studio'
                );
                if (closeBtn) closeBtn.click();
            }''')
            time.sleep(2)

            print(f"[{q}번 문제] 2. 소스 선택 모듈 실행...")
            page.evaluate('''() => {
            ...
            print(f"[{q}번 문제] 3. 질문 입력 및 추출 요청...")
            ...
            print(f"[{q}번 문제] 5. 가장 최근 메모를 열고 '{source_title}'로 이름 변경 후 소스로 변환...")
            try:
                page.evaluate('''() => {
            ...
            print(f"[{q}번 문제] 6. 스튜디오 슬라이드 생성 파이프라인 시작 (단일소스 선택)...")
            page.evaluate('''() => {
                let allCheck = document.querySelector('input[aria-label="모든 출처 선택"]');
                if (allCheck && allCheck.checked) allCheck.click();
                else {
            ...
            print(f"[{q}번 문제] 7. 슬라이드 이름 '{source_title}'로 동기화 처리 중...")
            try:
                page.evaluate(f'''() => {{
                ...
                print("슬라이드 이름 변경 실패:", e)
                
            time.sleep(2)
            print(f"✅ [{q}번 문제] 모든 작업 완료.")
